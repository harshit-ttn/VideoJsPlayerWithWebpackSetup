/*! @name videojs-contrib-eme @version 3.6.0 @license Apache-2.0 */
(function (global, factory) {
  false && typeof exports === "object" && typeof module !== "undefined"
    ? factory(
        exports,
    ...
    : false && typeof define === "function" && define.amd
    ? define(
        ["exports", "global/document", "video.js", "global/window"],
        factory
      )
    : factory(
        (global.videojsContribEme = {}),
        global.document,
        global.videojs,
        global.window
      );
})(this, function (exports, document, videojs, window) {
  "use strict";
  ...
  var arrayBuffersEqual = function arrayBuffersEqual(
    arrayBuffer1,
    arrayBuffer2
  ) {
  ...
  var arrayBufferFrom = function arrayBufferFrom(bufferOrTypedArray) {
    if (
      bufferOrTypedArray instanceof Uint8Array ||
  ...
  var requestPlayreadyLicense = function requestPlayreadyLicense(
    keySystemOptions,
    messageBuffer,
    emeOptions,
    callback
  ...
  var getSupportedConfigurations = function getSupportedConfigurations(
    keySystemOptions
  ) {
  ...
  var getSupportedKeySystem = function getSupportedKeySystem(keySystems) {
    ...
    Object.keys(keySystems).forEach(function (keySystem) {
      var supportedConfigurations = getSupportedConfigurations(
  ...
  var defaultPlayreadyGetLicense = function defaultPlayreadyGetLicense(
    keySystemOptions
  ) {
  ...
  var promisifyGetLicense = function promisifyGetLicense(
    getLicenseFn,
    eventBus
  ) {
  ...
  var standardizeKeySystemOptions = function standardizeKeySystemOptions(
    keySystem,
    keySystemOptions
  ) {
  ...
  var concatInitDataIdAndCertificate = function concatInitDataIdAndCertificate(
    _ref
  ) {
      ...
      if (!video.webkitKeys) {
        try {
          video.webkitSetMediaKeys(
            new window.WebKitMediaKeys(FAIRPLAY_KEY_SYSTEM)
          );
        } catch (error) {
  ...
  var fairplay = function fairplay(_ref3) {
    ...
    var getCertificate =
      fairplayOptions.getCertificate || defaultGetCertificate(fairplayOptions);
    var getContentId = fairplayOptions.getContentId || defaultGetContentId;
    var getLicense =
      fairplayOptions.getLicense || defaultGetLicense$1(fairplayOptions);
    return new Promise(function (resolve, reject) {
  ...
  var addKeyToSession = function addKeyToSession(
    options,
    session,
    event,
    eventBus
  ...
  var msPrefixed = function (_ref) {
    ...
    try {
      ...
      throw new Error(
        "Unable to create media keys for PlayReady key system. " +
          "Error: " +
          e.message
      );
    }
  ...
  var handleEncryptedEvent = function handleEncryptedEvent(
    event,
    options,
    sessions,
    eventBus
    ...
    return getSupportedKeySystem(options.keySystems).then(function (
      keySystemAccess
    ) {
  ...
  var handleWebKitNeedKeyEvent = function handleWebKitNeedKeyEvent(
    event,
    options,
    eventBus
  ) {
    if (
      !options.keySystems ||
      !options.keySystems[FAIRPLAY_KEY_SYSTEM] ||
      !event.initData
    ) {
      // return silently since it may be handled by a different system
      return Promise.resolve();
  ...
  var handleMsNeedKeyEvent = function handleMsNeedKeyEvent(
    event,
    options,
    sessions,
    eventBus
    ...
    if (
      sessions.reduce(function (acc, session) {
    ...
    if (
      options.keySystems[PLAYREADY_KEY_SYSTEM] &&
  ...
  var emeErrorHandler = function emeErrorHandler(player) {
    return function (objOrErr) {
      var message =
        typeof objOrErr === "string"
          ? objOrErr
          : (objOrErr && objOrErr.message) || null;
      player.error({
  ...
  var onPlayerReady = function onPlayerReady(player, emeError) {
    ...
    if (window.WebKitMediaKeys) {
      ...
      player.tech_.el_.addEventListener("webkitneedkey", function (event) {
        ...
        setupSessions(player);
        handleWebKitNeedKeyEvent(event, getOptions(player), player.tech_).catch(
          emeError
        );
      });
  ...
  var eme = function eme(options) {
    ...
    player.eme = {
      ...
      initializeMediaKeys: function initializeMediaKeys(
        ...
        // to encrypted events
        var mergedEmeOptions = videojs.mergeOptions(
